...
static bool	get_in(t_data *data, t_token *tmp, t_cmd *cmd)
{
	if (tmp->type == INPUT)
	{
		...
		cmd->infile = open_file(data, tmp->next->str, INPUT,&tmp->next->quote);
		if (cmd->infile == -1)
			return (false);
	}
	else if (tmp->type == HEREDOC)
	{
		...
		if (tmp == tmp->next || tmp->next->type <= 5)
			return (print_error_token(tmp, data));
		cmd->infile = open_file(data, tmp->next->str, HEREDOC,&tmp->next->quote);
		if (cmd->infile == -1)
			return (false);
	}
	return (true);
...
static bool	get_out(t_token *tmp, t_cmd *cmd, t_data *data)
{
	if (tmp->type == TRUNC)
	{
		...
		if (tmp == tmp->next || tmp->next->type <= 5)
			return (print_error_token(tmp, data));
		cmd->outfile = open_file(NULL, tmp->next->str, TRUNC,&tmp->next->quote);
		if (cmd->outfile == -1)
			return (false);
	}
	else if (tmp->type == APPEND)
	{
		...
		if (tmp == tmp->next || tmp->next->type <= 5)
			return (print_error_token(tmp, data));
		cmd->outfile = open_file(NULL, tmp->next->str, APPEND,&tmp->next->quote);
		if (cmd->outfile == -1)
			return (false);
	}
	return (true);
